...
1. **时间戳格式**: 10 位时间戳（秒级）会自动转换为毫秒级（乘以 1000）
2. **星期几**: `{a}` 占位符返回的是中文星期，周日起始（0=日，1=一，...，6=六）
3. **自动补零**: 当月份、日期、小时、分钟、秒小于 10 时，会自动在前面补 0
4. **字符串处理**: 日期字符串中的 `-` 会被替换为 `/`，`T` 会被替换为空格，毫秒部分会被移除

## 相关函数

- [timeFormat](./timeFormat) - 另一个时间格式化函数
